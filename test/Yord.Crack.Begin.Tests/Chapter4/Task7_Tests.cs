...
namespace Yord.Crack.Begin.Tests.Chapter4
{
    ...
    {
        [Test]
        public void Should_GetOrderDfs_Successfully()
        {
            var deps = new[]
            {
            ...
            var order = Task7.GetOrderDfs(new[] {'a', 'b', 'c', 'd', 'e', 'f'}, deps);
            
            CollectionAssert.AreEqual(new[] {'f', 'e', 'b', 'a', 'd', 'c'}, order);
        }
        
        [Test]
        public void ShouldNot_GetOrderDfs_WhenCycle()
        {
            var deps = new[]
            {
            ...
            var order = Task7.GetOrderDfs(new[] {'a', 'b', 'c', 'd'}, deps);
